function Set-TwinklyDeviceName{
    [CmdletBinding()]
    param(
    ...
    try{
        ...
        if($Verify.code -eq 1000){
            ...
            $Actions = @{'name' = $NewName}

            $Status = Invoke-RestMethod -Method Post -Uri "$uri/device_name" -Headers $Header -Body ($Actions | ConvertTo-Json)
            ...
            Write-Output $obj
                ...
                throw "Error setting name via API. Return Code: $($ModeStatus.Code)"
            }
        }
    }
    catch{
